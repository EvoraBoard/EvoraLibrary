...
EvoraBoard::EvoraBoard(Stream &modemStream, Stream &debugStream, ttn_fp_t fp, uint8_t sf, uint8_t fsb)
{
...
void EvoraBoard::debugPrintIndex(uint8_t index, const char *value)
{
  char message[100];
...
void EvoraBoard::debugPrintMessage(uint8_t type, uint8_t index, const char *value)
{
  char message[100];
  switch (type)
  {
...
void EvoraBoard::clearReadBuffer()
{
...
size_t EvoraBoard::readLine(char *buffer, size_t size)
{
  size_t read = 0;
  while (read == 0)
  {
...
size_t EvoraBoard::readResponse(uint8_t prefixTable, uint8_t index, char *buffer, size_t size)
{
  clearReadBuffer();
...
size_t EvoraBoard::readResponse(uint8_t prefixTable, uint8_t indexTable, uint8_t index, char *buffer, size_t size)
{
  clearReadBuffer();
  sendCommand(prefixTable, 0, true, false);
...
void EvoraBoard::reset(bool adr)
{
  autoBaud();
...
void EvoraBoard::onMessage(void (*cb)(const uint8_t *payload, size_t size, port_t port))
{
  messageCallback = cb;
}

bool EvoraBoard::personalize(const char *devAddr, const char *nwkSKey, const char *appSKey)
{
  reset(adr);
...
bool EvoraBoard::provision(const char *appEui, const char *appKey)
{
  reset(adr);
...
bool EvoraBoard::join(int8_t retries, uint32_t retryDelay)
{
  int8_t attempts = 0;
  configureChannels(fsb);
  setSF(sf);
...
bool EvoraBoard::join(const char *appEui, const char *appKey, int8_t retries, uint32_t retryDelay)
{
  return provision(appEui, appKey) && join(retries, retryDelay);
}

ttn_response_t EvoraBoard::sendBytes(const uint8_t *payload, size_t length, port_t port, bool confirm, uint8_t sf)
{
  if (sf != 0)
  {
    setSF(sf);
...
ttn_response_t EvoraBoard::poll(port_t port, bool confirm)
{
  uint8_t payload[] = {0x00};
  return sendBytes(payload, 1, port, confirm);
}

void EvoraBoard::showStatus()
{
  readResponse(SYS_TABLE, SYS_TABLE, SYS_GET_HWEUI, buffer, sizeof(buffer));
...
void EvoraBoard::configureEU868()
{
...
void EvoraBoard::configureUS915(uint8_t fsb)
{
  uint8_t ch;
...
void EvoraBoard::configureAU915(uint8_t fsb)
{
  uint8_t ch;
  uint8_t chLow = fsb > 0 ? (fsb - 1) * 8 : 0;
  uint8_t chHigh = fsb > 0 ? chLow + 7 : 71;
...
void EvoraBoard::configureAS920_923()
{
...
void EvoraBoard::configureAS923_925()
{
  /* RN2903AS 1.0.3rc9 defaults
   * CH0 = 923.2MHz
   * CH1 = 923.4MHz
...
void EvoraBoard::configureKR920_923()
{
  sendMacSet(MAC_ADR, "off"); // TODO: remove when ADR is implemented for this plan
...
void EvoraBoard::configureIN865_867()
{
  sendMacSet(MAC_ADR, "off"); // TODO: remove when ADR is implemented for this plan
...
void EvoraBoard::configureChannels(uint8_t fsb)
{
  switch (fp)
  {
  case TTN_FP_EU868:
...
bool EvoraBoard::setSF(uint8_t sf)
{
  uint8_t dr;
  switch (fp)
  {
...
void EvoraBoard::sendCommand(uint8_t table, uint8_t index, bool appendSpace, bool print)
{
  char command[100];
  switch (table)
  {
...
bool EvoraBoard::sendMacSet(uint8_t index, const char *value)
{
  clearReadBuffer();
  debugPrint(SENDING);
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
bool EvoraBoard::sendChSet(uint8_t index, uint8_t channel, const char *value)
{
  clearReadBuffer();
  char ch[5];
  if (channel > 9)
...
bool EvoraBoard::sendJoinSet(uint8_t type)
{
  clearReadBuffer();
  debugPrint(F(SENDING));
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
bool EvoraBoard::sendPayload(uint8_t mode, uint8_t port, uint8_t *payload, size_t length)
{
  clearReadBuffer();
  debugPrint(F(SENDING));
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
void EvoraBoard::sleep(uint32_t mseconds)
{
  if (mseconds < 100)
  {
    return;
...
void EvoraBoard::wake()
{ 
  autoBaud();
}

void EvoraBoard::linkCheck(uint16_t seconds)
{
  clearReadBuffer();
  debugPrint(SENDING);
  sendCommand(MAC_TABLE, MAC_PREFIX, true);
...
uint8_t EvoraBoard::getLinkCheckGateways()
{
  readResponse(MAC_TABLE, MAC_GET_SET_TABLE, MAC_GWNB, buffer, sizeof(buffer));
  return strtol(buffer, NULL, 10);
}

uint8_t EvoraBoard::getLinkCheckMargin()
{
